...
class Kiwoom(QAxWidget):
    ...
    def get_account_info(self):
        ...
        print("나의 보유 계좌번호 %s" % self.account_num) #계좌번호 -> 8149434311
    ...
    def trdata_slot(self, sScrNo, sRQName, sTrCode, sRecordName, sPrevNext):
        ...
        if sRQName == "계좌평가잔고내역요청":
            ...
            total_buy_money_result = str(total_buy_money)
            ...
            if sPrevNext =="2":
                ...
                if self.calcul_data == None or len(self.calcul_data) < 10:
                    pass_success = False
                    ...
                    total_price = 0
                    for value in self.calcul_data[:10]:
                        total_price += int(value[1])

                    moving_average_price = total_price / 10
                    ...
                    if bottom_stock_price == True:
                        ...
                        while True:
                            ...
                            if len(self.calcul_data[idx:]) < 10: #120일치가 있는지 계속 확인
                                print("20일치가 없음!")
                                break

                            total_price = 0
                            for value in self.calcul_data[idx:10+idx]:
                                total_price += int(value[1])
                            moving_average_price_prev = total_price / 10

                            if moving_average_price_prev <= int(self.calcul_data[idx][6]) and idx <= 2:    #20일정도는 밑에 있어야한다. (수정가능) 20일전에 고가가 이평선보다 위에 있으면 끊는다.
                                print("20일 동안 주가가 120일 이평선과 같거나 위에 있으면 조건 통과 못함")
                                price_top_moving = False
                                break

                            elif int(self.calcul_data[idx][7]) > moving_average_price_prev and idx > 2:
    ...
    def realdata_slot(self, sCode, sRealType, sRealData):
        ...
        if sRealType == "장시작시간":
            ...
            if sCode in self.account_stock_dict.keys() and sCode not in self.jango_dict.keys():
                ...
                if asd['매매가능수량'] > 0 and (meme_rate >= 3.5): # 수익이나거나 손절하는 부분(수정)
                    order_success = self.dynamicCall("SendOrder(QString, QString, QString, int, QString, int, int, QString, QString)",
                                    ["신규매도", self.portfolio_stock_dict[sCode], self.account_num, 2,
            ...
            # 오늘 산 잔고에 있을 경우
                ...
                if jd['주문가능수량'] > 0 and (meme_rate >= 3.5): # 수익이나 손절(수정)

                    order_success = self.dynamicCall(
                        "SendOrder(QString, QString, QString, int, QString, int, int, QString, QString)",
                        ["신규매도", self.portfolio_stock_dict[sCode], self.account_num, 2, sCode, jd['주문가능수량'],
            ...
            for order_num in not_meme_list:
                ...
                if order_gubun == "매수" and not_quantity > 0 and  e > meme_price:
                    order_success = self.dynamicCall(
                        "SendOrder(QString, QString, QString, int, QString, int, int, QString, QString)",
                        ["매수취소", self.portfolio_stock_dict[sCode], self.account_num, 3, code, 0, 0,
